...
class LogisticRegressionAlgorithm(object):
    ...
    def train_one_class(self, number_to_classify, inputs=None, labels=None, iterations=None, learning_rate=None):
        ...
        error_history = []
        print("Started training class {}\nT".format(number_to_classify))
    ...
    def train_all_classes(self, iterations=None, learning_rate=None):
        ...
        numbers_to_classify = range(1)
        with mp.Pool() as pool:
            weights = pool.map(self.train_one_class, numbers_to_classify)

        weights = np.reshape(weights, (784, 2))
        elapsed_time = time.time() - start_time
    ...
    def save_weights(self, weights, path=r"./multiclass_weights_part_b_multiprocess.npy"):
        """Save weights for further use"""
        np.save(path, weights)

    def get_weights_from_file(self, path=r"./multiclass_weights_part_b_multiprocess.npy"):
